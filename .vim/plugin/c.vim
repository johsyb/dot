...
function! C_RemoveCComment( start, end )
  ...
  let checkold = getline( a:end) =~ '^\s*\*\/'
  let checknew = getline( a:end) =~ '^\s*\*\(.*\)\*\/'
	...
	let check3 = check && getline(	linenumber ) =~ '^\s*\*[ ].*\*\/'
  if (!check3)
    let check2 = check && getline(	linenumber ) =~ '^\s*.*\*\/'
  else
    let check2 = 0
  endif
  if (!check2 && !check3) 
